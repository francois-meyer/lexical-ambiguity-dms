...
parser.add_argument("--setup", type=str, default="many2many")
parser.add_argument("--update", type=str, default="one")
parser.add_argument("--sim_metric", type=str, default="dot")
...
model = Word2DMMS(vocab_size, n_dim, m_dim, setup=setup, update=update, sim_metric=sim_metric, init=init)
...
def list_zeros():
    current_dms = model.get_density_matrices().cpu().numpy()
    for i, word in enumerate(vocab.itos):
        if not np.any(current_dms[i]):
            print("%s has all zeros" % word)
        elif not np.all(current_dms[i]):
            print("%s has some zeroes" % word)
...
B_target = model.get_component_matrices().cpu().numpy()
component_matrices = DensityMatrices(vocab, B_target)
component_matrices.save(output_path + "_comp")
print("Saved component matrices.")
